...
def test_sequence(args):
    ...
    print('----------\nOverall loss: {}/{}, avg ATE:{}, avg T_RTE:{}, avg D_RTE:{}'.format(
        np.average(losses_seq, axis=0), np.average(losses_avg), np.mean(ate_all), np.mean(t_rte_all), np.mean(d_rte_all)))
    print(ate_all, np.mean(ate_all))
    return losses_avg
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_dir', type=str, default="/data/INSData/ins_data_test/IDOL_SenseINS/building1/unknown", help='Path to data directory')
    parser.add_argument('--validation_dir', type=str, default="/data/INSData/ins_data_test/IDOL_SenseINS/building1/val_known")
    parser.add_argument('--cache_path', type=str, default=None, help='Path to cache folder to store processed data')
    parser.add_argument('--dataset', type=str, default='sense', choices=['ronin', 'ridi', 'sense'])
    ...
    parser.add_argument('--out_dir', type=str, default="../output/ronin_resnet/idol/2021.05.14/test_b1_unknown")
    parser.add_argument('--model_path', type=str, default="../output/ronin_resnet/idol/2021.05.14/train/checkpoints/checkpoint_91.pt")
